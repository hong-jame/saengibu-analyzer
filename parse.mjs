...
function parseCreative(rich, lines) {
  ...
  // OCR본은 "자치 활동"처럼 단어 중간에 공백을 넣는 경우가 있어 문자 사이 공백을 허용
  const areaLabel = /^(자\s*율\s*·\s*자\s*치\s*활\s*동|자\s*율\s*활\s*동|동\s*아\s*리\s*활\s*동|진\s*로\s*활\s*동)$/;
  ...
  // OCR본은 "자율·자치활동" 라벨이 "자율"+"자치활동"처럼 별개 토큰으로 쪼개지는 경우가 있어,
  // 같은 줄(Y 근접) 토큰을 왼쪽부터 이어붙여 보며 라벨 전체와 일치하는지 확인(정상 PDF는 첫 토큰에서 바로 일치).
  const findAreaLabelToks = pgTokens => {
    const byY = []; pgTokens.forEach(t => { const g = byY.find(r => Math.abs(r.y - t.y) <= 3); if (g) g.toks.push(t); else byY.push({ y: t.y, toks: [t] }); });
    const out = [];
    byY.forEach(row => {
      const toks = row.toks.slice().sort((a, b) => a.x - b.x);
      for (let i = 0; i < toks.length; i++) {
        let acc = '', prevEnd = null, j = i;
        for (; j < toks.length && j < i + 5; j++) {
          if (prevEnd != null && toks[j].x - prevEnd > 20) break; // 인접(작은 간격)까지만 병합
          acc += toks[j].s; prevEnd = toks[j].x + toks[j].w;
          if (areaLabel.test(acc.replace(/\s+/g, ''))) { out.push({ s: acc, x: toks[i].x, y: toks[i].y }); break; }
        }
      }
    });
    return out;
  ...
  for (let pi = startPage; pi < rich.length; pi++) {
    ...
    const labs = findAreaLabelToks(pg.tokens).sort((a, b) => b.y - a.y);
    labs.forEach(t => {
      // 같은 y의 중간열 숫자 = 시간(양식별 열 위치)
      const time = pg.tokens.find(n => n.x > hoursX[0] && n.x < hoursX[1] && /^\d{1,3}$/.test(n.s) && Math.abs(n.y - t.y) < 4);
      labelSeq.push({ page: pi, area: t.s.replace(/\s+/g, ''), y: t.y, hours: time ? +time.s : null });
    });
  ...
  // OCR본은 괘선(hlines)이 아예 없는 경우가 있음 → 그때는 라벨 Y좌표 자체를 밴드 경계로 사용(페이지 넘어도 직전 라벨 이어받음)
  const allLabelsOrdered = withYear.slice().sort((a, b) => a.page - b.page || b.y - a.y);
  const labelBandOf = (pi, y) => { let best = null; for (const L of allLabelsOrdered) { if (L.page < pi || (L.page === pi && L.y >= y - 1)) best = L; else break; } return best; };
  ...
  for (let pi = startPage; pi < rich.length; pi++) {
    ...
    for (const r of rows) {
      ...
      let info;
      if (borders.length) { const t = bandTop(r.y, borders); info = t != null ? map[t] : null; }
      else { const L = labelBandOf(pi, r.y); info = L ? { area: areaKey[L.area], year: L.year } : null; }
      if (!info) continue;
...
function parseSubjects(rich) {
  ...
  for (let pi = startPage; pi < rich.length && !done; pi++) {
    ...
    for (const row of rows) {
      ...
      if ((m = l.match(/^\[\s*([1-3])\s*학년\s*\]/))) { grade = +m[1]; mode = 'score'; curDet = null; continue; }
      ...
      if (mode === 'score') {
        ...
        // 성적표가 전부 인식 실패(예: OCR 손상)면 subjNames가 비어 있어 세특까지 통째로 유실됨 →
        // 이 경우에 한해(정상 케이스는 그대로) 과목명 사전확인 없이도 헤더로 인정(정규식의 문자 제한이 이미 산문 오탐을 걸러줌).
        if (dm && (isKnownSubject(dm[1].trim()) || subjNames.size === 0)) { curDet = { grade, subject: dm[1].trim(), text: dm[2].trim(), _sp: row.sp }; details.push(curDet); }
