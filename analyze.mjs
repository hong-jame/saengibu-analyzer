...
export function gist(sen, n = 42) {
  ...
  let best = null, bs = -1;
  clauses.forEach(c => {
    const sc = STRONG_MARKERS.filter(m => c.includes(m)).length * 2 + WEAK_MARKERS.filter(m => c.includes(m)).length;
...
export function extractPositive(text) {
  ...
  const out = [];
  sents.forEach(sen => {
    const strong = [...new Set(STRONG_MARKERS.filter(m => sen.includes(m)))];
    const weak = [...new Set(WEAK_MARKERS.filter(m => sen.includes(m)))];
...
export function competencySignals(r) {
  ...
  return Object.entries(COMPETENCY).map(([key, def]) => {
    const subs = def.subs.map(sub => {
      ...
      sents.forEach(se => {
        const hits = sub.kws.filter(k => se.t.includes(k));
...
export function actionVerbs(r) {
  ...
  const cats = ACTION_VERBS.map(av => {
    ...
    sents.forEach(se => {
      const hit = av.kws.find(k => se.t.includes(k)); if (!hit) return;
